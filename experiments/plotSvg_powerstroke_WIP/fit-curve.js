// ===================================================================
// From https://github.com/soswow/fit-curve/blob/master/src/fit-curve.js
// Has some small changes to avoid name collisions with p5.js
 ...
 * @returns {Array<Array<Array<Number>>>} Array of Bezier curves, where each element
 * is [first-point, control-point-1, control-point-2, second-point] and points are [x, y]
 */
 ...
 * @returns {Array<Array<Array<Number>>>} Array of Bezier curves, where each element is
 * [first-point, control-point-1, control-point-2, second-point] and points are [x, y]
 */
function fitCubic(points, leftTangent, rightTangent, error, progressCallback) {
  ...
  var bezCurve, // Control points of fitted Bezier curve
    ...
    centerVector,
    toCenterTangent,
    fromCenterTangent, // Unit tangent vector(s) at splitPoint
 ...
 * [first-point, control-point-1, control-point-2, second-point] where points are [x, y]
 */
function generateBezier(points, parameters, leftTangent, rightTangent) {
  var bezCurve, //Bezier curve ctl pts
    ...
    det_C0_C1,
    det_C0_X,
    det_X_C1, // Determinants of matrices
    alpha_l,
    alpha_r, // Alpha values, left and right
    epsilon,
    segLength,
    i,
    len,
  ...
  for (i = 0, len = points.length; i < len; i++) {
    ...
    tmp = maths.subtract(
      points[i],
      fcbezier.q([firstPoint, firstPoint, lastPoint, lastPoint], u)
    );
 ...
 */
function reparameterize(curBezier, points, parameters) {
  /*
    var j, len, point, results, u;
    results = [];
    for (j = 0, len = points.length; j < len; j++) {
        point = points[j], u = parameters[j];
        results.push(newtonRaphsonRootFind(curBezier, point, u));
    }
    return results;
    //*/
  return parameters.map((p, i) =>
    newtonRaphsonRootFind(curBezier, points[i], p)
  );
}
...
function newtonRaphsonRootFind(bez, point, u) {
  ...
  var d = maths.subtract(fcbezier.q(bez, u), point),
    qprime = fcbezier.qprime(bez, u),
    numerator = maths.mulMatrix(d, qprime),
    denominator =
      maths.sum(maths.squareItems(qprime)) +
      2 * maths.mulMatrix(d, fcbezier.qprimeprime(bez, u));
...
function computeMaxError(points, bez, parameters) {
  ...
  for (i = 0, count = points.length; i < count; i++) {
    ...
    v = maths.subtract(fcbezier.q(bez, t), point);
...
var mapTtoRelativeDistances = function (bez, B_parts) {
  ...
  for (var i = 1; i <= B_parts; i++) {
    B_t_curr = fcbezier.q(bez, i / B_parts);
...
const amap = Array.prototype.map;
...
class maths {
  ...
  static mulItems(items, multiplier) {
    ...
    // return amap.call(items, (x) => x * multiplier);
  }
  ...
  static subtract(arr1, arr2) {
    ...
    // return amap.call(arr1, (x1, i) => x1 - arr2[i]);
  }
  ...
  // Only used on two arrays. 
  // The dot product is equal to the matrix product in this case:
