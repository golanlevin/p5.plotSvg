...
(function(global) {
  ...
  // Internal recording state. This first batch contains low-risk containers
  // and counters; lifecycle, p5 instance, and override state remain separate.
  const session = {
    commands: [],
  ...
  p5plotSvg.beginRecordSvg = function(p5InstanceOrFilename, maybeFilename) {
    ...
    session.vertexStack = [];
    session.groupStack = [];
    session.hasNonDefaultStrokeColor = false;
    session.injectedHeaderAttributes = [];
    session.injectedDefs = [];
    session.groupLevel = 0;
    ...
    session.currentStrokeColor = config.defaultStrokeColor;
    _inkscapeLayerMap = {};
    _inkscapeUsedLabels = {};
  ...
  */
  ...
  function exportSVG() {
    ...
    for (let attr of session.injectedHeaderAttributes) {
          ...
          if (cmd.type === SVG_COMMAND.PUSH) {
            ...
            if (transformGroupStack.length > 0) {
              while (transformGroupStack[transformGroupStack.length - 1] > 0){
                ...
                session.groupLevel = Math.max(0, session.groupLevel - 1);
                svgContent += getIndentStr();
                svgContent += `</g>\n`;
              }
  ...
  function getIndentStr(depthOverride) {
    let out = '';
    let depth = (typeof depthOverride === 'number') ? depthOverride : session.groupLevel;
  ...
  function overrideClipFunction() {
    ...
    const newClipFunc = function() {
      if (session.recording && !_bClipWarningShown) {
  ...
  function getSvgStrStroke() {
    ...
    if (config.groupByStrokeColor || session.hasNonDefaultStrokeColor || !areColorsEqual(session.currentStrokeColor, config.defaultStrokeColor)) {
      let colorStr = session.currentStrokeColor;
