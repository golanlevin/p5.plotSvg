...
let _svgIndentType = SVG_INDENT_SPACES; 
 ...
 */
function beginRecordSVG(p5Instance, fn) {
 ...
 */
function pauseRecordSVG(bPause){
 ...
 */
function setSvgDefaultStrokeWeight(wei) {
 ...
 */
function setSvgDefaultStrokeColor(col) {
 ...
 */
function setSvgResolutionDPI(dpi) {
 ...
 */
function setSvgIndent(itype, inum) {
  // Set indent type if it matches one of the predefined constants
  if (itype === SVG_INDENT_SPACES || itype === SVG_INDENT_TABS || itype === SVG_INDENT_NONE) {
  ...
  if (inum !== undefined && Number.isInteger(inum) && inum >= 0) {
    if (_svgIndentType === SVG_INDENT_NONE){
      _svgIndentAmount = 0; 
    } else {
    ...
    // Defaults
    if (_svgIndentType === SVG_INDENT_SPACES){
      _svgIndentAmount = 2; 
    } else if (_svgIndentType === SVG_INDENT_TABS){
      _svgIndentAmount = 1; 
    } else if (_svgIndentType === SVG_INDENT_NONE){
      _svgIndentAmount = 0; 
    }
  }
}
...
function getIndentStr(){
  let out = ''; 
  if ((_svgIndentType != SVG_INDENT_NONE) && (_svgIndentAmount > 0)){
    let c = (_svgIndentType === SVG_INDENT_SPACES) ? ' ' : '\t'; 
    if (_svgGroupLevel > 0){
 ...
 */
function setSvgFlattenTransforms(b){
  if (b === true){
 ...
 */
function setSvgCoordinatePrecision(p) {
 ...
 * Must be a non-negative integer. If an invalid value is provided, a warning is issued.
 */
function setSvgTransformPrecision(p) {
  // Check if p` is a number and an integer
  if (typeof p === 'number' && Number.isInteger(p) && (p >= 0)) {
 ...
 */
function setSvgPointRadius(radius) {
