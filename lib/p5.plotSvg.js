...
(function(global) {
  ...
  let _svgCurrentStrokeColor = config.defaultStrokeColor;
  ...
  function overrideFunction(target, funcName, newFunc) {
    ...
    if (target === globalTarget && !_bUsingGlobalMode) return;

    let restoreInfo = _overrideRestoreStack.find(item =>
      item.target === target && item.funcName === funcName
    );
    if (!restoreInfo) {
      const hadOwnProperty = Object.prototype.hasOwnProperty.call(target, funcName);
      _overrideRestoreStack.push({
        target,
        funcName,
        hadOwnProperty,
  ...
  function restoreOverriddenFunctions() {
    for (let i = _overrideRestoreStack.length - 1; i >= 0; i--) {
      const item = _overrideRestoreStack[i];
   ...
   */
  function getP5Version(p5Instance = _p5Instance) {
  ...
  p5plotSvg.beginRecordSvg = function(p5InstanceOrFilename, maybeFilename) {
    ...
    if (_p5MajorVersion >= 2 && _bUsingGlobalMode && typeof p5 !== 'undefined' && typeof p5.instance !== 'undefined') {
    ...
    _svgCurrentStrokeColor = config.defaultStrokeColor;
    _inkscapeLayerMap = {};
    _inkscapeUsedLabels = {};
  ...
  function overrideArcFunction() {
    ...
    const newArcFunc = function(x, y, w, h, start, stop, mode, detail = 0) {
      if (_bRecordingSvg) {
        ...
        _commands.push({ type: SVG_COMMAND.ARC, x, y, w, h, start, stop, mode: svgMode, transformMatrix });
      }
  ...
  function overrideBezierFunction(){
    ...
    const newBezierFunc = function(x1, y1, x2, y2, x3, y3, x4, y4) {
      if (_bRecordingSvg) {
        let transformMatrix = captureCurrentTransformMatrix();
        _commands.push({ type: SVG_COMMAND.BEZIER, x1, y1, x2, y2, x3, y3, x4, y4, transformMatrix });
      }
  ...
  function overrideCircleFunction(){
    ...
    const newCircleFunc = function(x, y, d) {
      ...
      if (_bRecordingSvg) {
        ...
        if (ellipseMode === 'center'){
          _commands.push({ type: SVG_COMMAND.CIRCLE, x, y, d, transformMatrix });
        } else if (ellipseMode === 'corner'){
          x += d/2;
          y += d/2;
          _commands.push({ type: SVG_COMMAND.CIRCLE, x, y, d, transformMatrix });
          ...
          x += w/2;
          y += h/2;
          _commands.push({ type: SVG_COMMAND.ELLIPSE, x, y, w, h, transformMatrix });
        }
      }
    ...
    if (_p5MajorVersion >= 2) {
      const curveCompatFunc = function(x1, y1, x2, y2, x3, y3, x4, y4) {
        ...
        if (_bRecordingSvg) {
          ...
          _commands.push({ type: SVG_COMMAND.CURVE, x1, y1, x2, y2, x3, y3, x4, y4, tightness, transformMatrix });
        }
  ...
  */
  ...
  function overrideLineFunction() {
    ...
    const newLineFunc = function(x1, y1, x2, y2) {
      if (_bRecordingSvg) {
        let transformMatrix = captureCurrentTransformMatrix();
        _commands.push({ type: SVG_COMMAND.LINE, x1, y1, x2, y2, transformMatrix });
      }
  ...
  function overridePointFunction() {
    ...
    const newPointFunc = function(x, y) {
      if (_bRecordingSvg) {
        let transformMatrix = captureCurrentTransformMatrix();
        _commands.push({ type: SVG_COMMAND.POINT, x, y, radius: config.pointRadius, transformMatrix });
      }
  ...
  function overrideQuadFunction(){
    ...
    const newQuadFunc = function(x1, y1, x2, y2, x3, y3, x4, y4) {
      if (_bRecordingSvg) {
        let transformMatrix = captureCurrentTransformMatrix();
        _commands.push({ type: SVG_COMMAND.QUAD, x1, y1, x2, y2, x3, y3, x4, y4, transformMatrix });
      }
    ...
    const newRectFunc = function(x, y, w, h, tl, tr, br, bl) {
      ...
      if (_bRecordingSvg) {
        ...
        if (arguments.length === 5) { // Single corner radius
          _commands.push({ type: SVG_COMMAND.RECT, x, y, w, h, tl, transformMatrix });
        } else if (arguments.length === 8) { // Individual corner radii
          _commands.push({ type: SVG_COMMAND.RECT, x, y, w, h, tl,tr,br,bl, transformMatrix });
        } else { // Standard rectangle
          _commands.push({ type: SVG_COMMAND.RECT, x, y, w, h, transformMatrix });
        }
      }
  ...
  function overrideTriangleFunction(){
    ...
    const newTriangleFunc = function(x1, y1, x2, y2, x3, y3) {
      if (_bRecordingSvg) {
        let transformMatrix = captureCurrentTransformMatrix();
        _commands.push({ type: SVG_COMMAND.TRIANGLE, x1, y1, x2, y2, x3, y3, transformMatrix });
      }
  ...
  function overrideBezierDetailFunction() {
    ...
    overrideFunction(_p5Instance, 'bezierDetail', newBezierDetailFunc);
  ...
  function overrideCurveTightnessFunction() {
    ...
    overrideFunction(_p5Instance, 'curveTightness', newCurveTightnessFunc);
  ...
  function overrideVertexFunction() {
    ...
    const newVertexFunc = function(x, y) {
      if (_bRecordingSvg) {
        _vertexStack.push({ type: SVG_SEGMENT.VERTEX, x, y });
      }
    ...
    const newBezierVertexFunc = function(x2, y2, x3, y3, x4, y4) {
      if (_bRecordingSvg) {
        ...
        if (_p5MajorVersion >= 2 && arguments.length <= 5) {
          ...
          _vertexStack.push({ type: SVG_SEGMENT.BEZIER, x2, y2, x3, y3, x4, y4 });
        }
      }
  ...
  function overrideQuadraticVertexFunction() {
    ...
    const newQuadraticVertexFunc = function(cx, cy, x, y) {
      if (_bRecordingSvg) {
        _shapeMode = 'complex'; // Switch to complex mode
        _vertexStack.push({ type: SVG_SEGMENT.QUADRATIC, cx, cy, x, y });
      }
      if (_p5MajorVersion >= 2) {
      ...
      if (_bRecordingSvg) {
        ...
        if (bDoKludge){
          if (_vertexStack.length === 1){
            if(_vertexStack[0].type === SVG_SEGMENT.CURVE){
              let x0 = _vertexStack[0].x;
              let y0 = _vertexStack[0].y;
              let dist01 = Math.hypot(x-x0, y-y0);
              if (dist01 > 0){
                _vertexStack.shift();
                _vertexStack.push({ type: SVG_SEGMENT.CURVE, x, y, tightness });
              }
            }
          }
        }
  ...
  function overrideBeginContourFunction() {
    ...
    const newBeginContourFunc = function() {
      if (_bRecordingSvg) {
        _vertexStack.push({ type: SVG_SEGMENT.CONTOUR_START });
      }
  ...
  function overrideEndContourFunction() {
    ...
    const newEndContourFunc = function(mode) {
      if (_bRecordingSvg) {
        ...
        _vertexStack.push({ type: SVG_SEGMENT.CONTOUR_END, closed: isClosed });
      }
      ...
      if (_bRecordingSvg && _vertexStack.length > 0) {
        ...
        switch (_shapeKind) {
          case SVG_COMMAND.POINTS:
            _commands.push({ type: SVG_COMMAND.POINTS, vertices: [..._vertexStack], transformMatrix });
            break;
          case _p5Instance.LINES:
            _commands.push({ type: SVG_COMMAND.LINES, vertices: [..._vertexStack], transformMatrix });
            break;
          case _p5Instance.TRIANGLES:
            _commands.push({ type: SVG_COMMAND.TRIANGLES, vertices: [..._vertexStack], transformMatrix });
            break;
          case _p5Instance.TRIANGLE_FAN:
            _commands.push({ type: SVG_COMMAND.TRIANGLE_FAN, vertices: [..._vertexStack], transformMatrix });
            break;
          case _p5Instance.TRIANGLE_STRIP:
            _commands.push({ type: SVG_COMMAND.TRIANGLE_STRIP, vertices: [..._vertexStack], transformMatrix });
            break;
          case _p5Instance.QUADS:
            _commands.push({ type: SVG_COMMAND.QUADS, vertices: [..._vertexStack], transformMatrix });
            break;
          case _p5Instance.QUAD_STRIP:
            _commands.push({ type: SVG_COMMAND.QUAD_STRIP, vertices: [..._vertexStack], transformMatrix });
            break;
          ...
          default:
            ...
            let hasContours = _vertexStack.some(v => v.type === SVG_SEGMENT.CONTOUR_START);
    ...
    const newDescribeFunc = function(description) {
      if (_bRecordingSvg) {
        if (description && description.trim().length > 0){
          ...
          _commands.push({ type: SVG_COMMAND.DESCRIPTION, text: description });
        }
      }
  ...
  function overridePushFunction(){
    ...
    const newPushFunc = function() {
      if (_bRecordingSvg) {
        _commands.push({ type: SVG_COMMAND.PUSH });
      }
  ...
  function overridePopFunction(){
    ...
    const newPopFunc = function() {
      if (_bRecordingSvg) {
        _commands.push({ type: SVG_COMMAND.POP });
      }
  ...
  function overrideScaleFunction(){
    ...
    const newScaleFunc = function(sx, sy) {
      if (_bRecordingSvg) {
        _commands.push({ type: SVG_COMMAND.SCALE, sx, sy: sy || sx });
      }
  ...
  function overrideTranslateFunction(){
    ...
    const newTranslateFunc = function(tx, ty) {
      if (_bRecordingSvg) {
        _commands.push({ type: SVG_COMMAND.TRANSLATE, tx, ty });
      }
  ...
  function overrideRotateFunction(){
    ...
    const newRotateFunc = function(angle) {
      if (_bRecordingSvg) {
        _commands.push({ type: SVG_COMMAND.ROTATE, angle });
      }
  ...
  function overrideShearXFunction(){
    ...
    const newShearXFunc = function(angle) {
      if (_bRecordingSvg) {
        _commands.push({ type: SVG_COMMAND.SHEAR_X, angle });
      }
  ...
  function overrideShearYFunction(){
    ...
    const newShearYFunc = function(angle) {
      if (_bRecordingSvg) {
        _commands.push({ type: SVG_COMMAND.SHEAR_Y, angle });
      }
  ...
  function exportSVG() {
    ...
    if (_groupStack.length > 0) {
      ...
      while (_groupStack.length > 0) {
        _commands.push({ type: SVG_COMMAND.END_GROUP });
        _groupStack.pop();
      }
    }
    ...
    if (config.inkscapeCompatibility) {
      const hasGroups = _commands.some(cmd => cmd.type === SVG_COMMAND.BEGIN_GROUP);
    ...
    let svgW = config.customSizeSet ? config.width : _p5Instance.width;
    let svgH = config.customSizeSet ? config.height : _p5Instance.height;
    ...
    for (let attr of _injectedHeaderAttributes) {
      const attrName = normalizeXmlName(attr.name, 'SVG header attribute name');
      if (attrName) {
        svgContent += ` ${attrName}="${escapeXmlAttribute(attr.value)}" `;
      }
    }
    ...
    if (_injectedDefs.length > 0) {
      ...
      for (let def of _injectedDefs) {
        ...
        for (let attr of def.attributes) {
          const attrName = normalizeXmlName(attr.name, 'SVG defs attribute name');
          if (attrName) {
            svgContent += `${attrName}="${escapeXmlAttribute(attr.value)}" `;
          }
        }
    ...
    for (let cmd of _commands) {
      ...
      if (cmd.type === SVG_COMMAND.PUSH ||
        ...
        if (!config.flattenTransforms && _bTransformsExist) {
          if (cmd.type === SVG_COMMAND.PUSH) {
            // Open a new group
  ...
  function getSvgStrBeginGroup(cmd) {
    ...
    if (Array.isArray(cmd.attributes)) {
      for (let attr of cmd.attributes) {
        ...
        const attrName = normalizeXmlName(attr.name, 'SVG group attribute name');
        if (attrName) {
          attrStr += ` ${attrName}="${escapeXmlAttribute(attr.value)}"`;
        }
      }
    }
  ...
  function getSvgStrMultiContourPath(cmd, styleStr, transformStr) {
    ...
    if (Array.isArray(cmd.attributes)) {
      for (let attr of cmd.attributes) {
        const attrName = normalizeXmlName(attr.name, 'SVG path attribute name');
        if (attrName) {
          str += ` ${attrName}="${escapeXmlAttribute(attr.value)}"`;
        }
      }
    }
    str += `/>\n`;
  ...
  function generateComplexContourPath(contour) {
    ...
    if (verts.every(v => v.type === SVG_SEGMENT.SPLINE)) {
  ...
  function getSvgStrPoly(cmd) {
    ...
    if (cmd.type === SVG_COMMAND.POLYLINE) {
      ...
      if (Array.isArray(cmd.attributes)) {
        for (let attr of cmd.attributes) {
          const attrName = normalizeXmlName(attr.name, 'SVG polyline attribute name');
          if (attrName) {
            str += ` ${attrName}="${escapeXmlAttribute(attr.value)}"`;
          }
        }
      ...
      let d = '';
      if (cmd.segments.every(seg => seg.type === SVG_SEGMENT.SPLINE)) {
  ...
  function captureCurrentTransformMatrix() {
    if (_bTransformsExist && config.flattenTransforms) {
  ...
  function overrideStrokeFunction() {
    ...
    const newStrokeFunc = function(...args) {
      if (_bRecordingSvg) {
        ...
        _commands.push({ type: SVG_COMMAND.STROKE, color: scol });
      }
  ...
  function getSvgStrStroke() {
    ...
    if (config.groupByStrokeColor || _hasNonDefaultStrokeColor || !areColorsEqual(_svgCurrentStrokeColor, config.defaultStrokeColor)) {
