...
// v.0.1.5, June 26, 2025
// Known to work with p5.js versions 1.4.2–1.11.8
...
(function(global) {
  ...
  */
   ...
   * Optionally assigns a group name (as an SVG ID).
   * The name must be a non-empty string to be used as an ID;
   * otherwise, the group will be anonymous.
  ...
  p5plotSvg.beginSvgGroup = function(gname) {
    if (typeof gname === 'string') {
      ...
      if (trimmedName.length > 0) {
        _commands.push({ type: 'beginGroup', gname: trimmedName });
        return;
      } else {
        console.warn('Warning: Ignoring empty string passed to beginSvgGroup(). Group will be anonymous.');
      }
    }
    // Fall back to anonymous group
