...
(function(global) {
  ...
  const session = {
    ...
    layers: {
      inkscapeLayerMap: {},       // Maps group names to layer numbers
      inkscapeUsedLabels: {},     // Tracks used label values for collision detection
  ...
  function resetSessionLayers() {
    session.layers.inkscapeLayerMap = {};
    session.layers.inkscapeUsedLabels = {};
  ...
  */
  ...
  function getOrAssignInkscapeLayerNumber(gname) {
    if (!gname) {
      ...
      while (session.layers.inkscapeUsedLabels.hasOwnProperty(String(session.layers.inkscapeNextLayerNumber))) {
      ...
      session.layers.inkscapeUsedLabels[String(num)] = '(unnamed group)';
      session.layers.inkscapeUsedLabels[labelValue] = '(unnamed group)';
      session.layers.inkscapeNextLayerNumber++;
      return num;
    }

    // Named groups: check if we've seen this name before
    if (session.layers.inkscapeLayerMap.hasOwnProperty(gname)) {
      return session.layers.inkscapeLayerMap[gname];
    }
    ...
    session.layers.inkscapeLayerMap[gname] = num;
    // Store both the number (for skip logic) and full label (for collision detection)
    session.layers.inkscapeUsedLabels[String(num)] = gname;
    session.layers.inkscapeUsedLabels[labelValue] = gname;
    session.layers.inkscapeNextLayerNumber++;
    return num;
  }
  ...
  function checkInkscapeLabelCollision(label, gname) {
    ...
    const existingOwner = session.layers.inkscapeUsedLabels[labelStr];
    ...
    if (session.recording) {
      ...
      if (config.inkscapeCompatibility) {
        ...
        if (userProvidedInkscapeLabel !== null) {
          ...
          session.layers.inkscapeUsedLabels[userProvidedInkscapeLabel] = group.gname || '(unnamed group)';
        } else {
