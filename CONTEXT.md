...
- **Version**: 0.3.0
- **Primary source file**: `lib/p5.plotSvg.js` (~4,900 lines)
...
| `inkscapeCompatibility` | `true` | Add Inkscape layer metadata for named groups |
| `exportPolylinesAsPaths` | `false` | Export simple polylines as `<path>` instead of `<polyline>` |
| `exportMalformedNumbersWithSanitizations` | `true` | Sanitize malformed numeric values instead of dropping affected elements |
| `clampLargeCoordinates` | `true` | Clamp very large finite coordinates to a configured magnitude |
| `coordinateClampMagnitude` | `1000000` | Absolute coordinate clamp limit when large-coordinate clamping is active |
...
- `formatNumber()` centralizes numeric formatting and guards against `NaN` /
  `Infinity`.
- Malformed numeric values are sanitized by default; callers may instead omit
  affected elements with `setSvgExportMalformedNumbersWithSanitizations(false)`.
- Very large finite coordinates are clamped by default to `[-1000000, 1000000]`,
  configurable through `setSvgClampLargeCoordinates()` and
  `setSvgCoordinateClampMagnitude()`.
- Degenerate no-op paths, including empty paths, move-only paths, and
  move/close-only paths, are omitted from exported SVG.
...
- `setSvgExportMalformedNumbersWithSanitizations(bool)`
- `setSvgClampLargeCoordinates(bool)`
- `setSvgCoordinateClampMagnitude(magnitude)`
...
- Text/font behavior is not fully v2-aware, especially for loaded fonts and
  vertical alignment.
- `examples/plotSvg_smorgasbord/` remains the main example that needs special
  version-aware attention because it exercises curve behavior.
...
thin build entries that import the library and expose browser/ESM outputs.
`dist/` is generated by `npm run build` and should be committed for releases.

This is an intentional conservative packaging policy for the add-on release:
preserve the current stable implementation and legacy CDN paths while adding
official p5 add-on distribution surfaces. A later structural cleanup may move
implementation source into `src/` and make both `lib/` and `dist/` generated,
but that migration is deferred until after the add-on packaging and release are
stable.
...
├── admin/
│   └── p5-libraries-listing/  # Staging materials for the p5 Libraries PR
├── examples/                  # Example sketches
├── experiments/               # Archived/WIP experiments, not public examples
├── test/                      # Manual fixtures and Playwright tests
├── images/                    # README images
├── documentation.md           # API documentation
├── README.md                  # Project overview
├── ROADMAP.md                 # Release roadmap and deferred work
├── rollup.config.mjs
├── package-lock.json
└── package.json
```
...
The `admin/p5-libraries-listing/` directory contains local staging copies for a
future p5 website PR. In the p5 website repository, the YAML belongs at
`src/content/libraries/en/p5.plotSvg.yaml` and the image belongs at
`src/content/libraries/images/p5.plotSvg.png`.
...
npx playwright test test/p5-addon-example.spec.js --browser=chromium
...
As of the 0.3.0 release-prep pass, `npm test` runs 25 Chromium tests.
...
- `test/p5-addon-example.spec.js`: verifies the four public add-on examples
  across p5 v1/v2 and global/instance mode.
